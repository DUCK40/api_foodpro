...
async function getCart(req, res, ip) {
  try {
    ...
    q = `SELECT
        ...
        "T_SALEORDER_BILLING_SEND"."DELIVERY_DOC_REF",
        "T_SALEORDER_BILLING_SEND"."DELIVERY_DOC_TRACK"
        ...
        AND "T_SALEORDER_BILLING_ITEM"."MEM_CODE" = "T_SALEORDER_HEADER"."MEM_CODE" and "T_SALEORDER_BILLING_ITEM"."BILL_STATUS" = 3
    ...
    rows.map((data, index) => {
      ...
      var DELIVERY_DOC_REF = Date.now().toString();
      var DELIVERY_DOC_TRACK = Date.now().toString();
      var date = new Date();
      ...
      var TESTSENT =
        ...
        arrResult.push({
          ...
          DELIVERY_DOC_REF: DELIVERY_DOC_REF,
          DELIVERY_DOC_TRACK: DELIVERY_DOC_TRACK,
          SENDDATE : TESTSENT
        });
    });
    if (arrResult.length > 0) {
      ...
      if (arrResult.length == 1) {
        detail.push({
          ...
          DELIVERY_DOC_REF: arrResult[0]["DELIVERY_DOC_REF"],
          DELIVERY_DOC_TRACK: arrResult[0]["DELIVERY_DOC_TRACK"],
          SENDDATE : arrResult[0]["SENDDATE"]
        });
        sum_so_item =
        ...
        for (var i = 0; i < arrResult.length; i++) {
          if (i == 0) {
            detail.push({
              ...
              DELIVERY_DOC_REF: arrResult[i]["DELIVERY_DOC_REF"],
              DELIVERY_DOC_TRACK: arrResult[i]["DELIVERY_DOC_TRACK"],
              SENDDATE : arrResult[i]["SENDDATE"]
            });
            sum_so_item =
...
async function updateData(req, res, ip) {
  try {
    ...
    qArr = [];
    pArr = [];
    //step to delivery bill status 3 > confirm รับ เป็น 4
