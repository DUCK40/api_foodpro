...
async function getDetail(shop_code, res, ip) {
  try {
    const shopArr = [];
    q = `SELECT TBI."SHOP_CODE",COUNT(TBI."BILL_ITEM_CODE") as TOTAL
    ...
    if (rows.length > 0) {
      rows.map((data, index) => {
        shopArr.push({ SHOP_CODE: data["SHOP_CODE"], TOTAL: data["total"] });
      });
    } else {
      shopArr.push({ SHOP_CODE: shop_code, TOTAL: 0 });
    }
